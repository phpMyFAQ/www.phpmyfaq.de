...
export async function generateMetadata({ params }: SecurityAdvisoryPageProps): Promise<Metadata> {
  ...
  if (!advisoryData) {
    return generatePageMetadata('Security Advisory Not Found', 'The requested security advisory could not be found');
  }

  return generatePageMetadata(advisoryData.title, advisoryData.description);
}
